...
func TestScheduler_BackpressureKeepsBoundedQueueLossless(t *testing.T) {
	t.Parallel()
	...
	s := NewBackpressuredSchedulerWithContext(context.Background(), 2, 1, func(ctx context.Context, n int) error {
		if n == 1 {
			close(started)
			<-release
		}
	...
	require.NoError(t, s.AddWork(t.Context(), "hot", 1))
	<-started
	require.NoError(t, s.AddWork(t.Context(), "hot", 2))

	thirdDone := make(chan error, 1)
	go func() {
		thirdDone <- s.AddWork(t.Context(), "hot", 3)
	}()
	select {
	case err := <-thirdDone:
		t.Fatalf("third AddWork returned before queue space was available: %v", err)
...
func TestScheduler_BackpressureWaitRespectsContext(t *testing.T) {
	t.Parallel()
	...
	defer cancel()
	require.ErrorIs(t, s.AddWork(ctx, "hot", 3), context.DeadlineExceeded)
}
