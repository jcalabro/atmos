...
func (s *ConvoGetMessages_Output) AppendJSON(buf []byte) ([]byte, error) {
	buf = append(buf, '{')
	first := true
	if s.LexiconTypeID != "" {
		if !first {
		...
		buf = append(buf, jsonKey_ConvoGetMessages_Output_dollar_type...)
	...
	if s.Cursor.HasVal() {
		...
		buf = append(buf, jsonKey_ConvoGetMessages_Output_cursor...)
	...
	buf = append(buf, jsonKey_ConvoGetMessages_Output_messages...)
	buf = append(buf, '[')
...
func (s *ConvoGetMessages_Output) UnmarshalJSON(data []byte) error {
	_, err := s.UnmarshalJSONAt(data, 0)
	return err
}

func (s *ConvoGetMessages_Output) UnmarshalJSONAt(data []byte, pos int) (int, error) {
	s.extraJSON = nil
	var err error
		...
		case "messages":
			if !cbor.IsJSONNull(data, pos) {
				...
				for {
					...
					var elem ConvoGetMessages_Output_Messages
...
func (s *ConvoGetMessages_Output) AppendCBOR(buf []byte) ([]byte, error) {
	n := 1 + len(s.extraCBOR)
	if s.LexiconTypeID != "" {
		n++
	}
...
func (s *ConvoGetMessages_Output) UnmarshalCBOR(data []byte) error {
	_, err := s.UnmarshalCBORAt(data, 0)
	return err
}

func (s *ConvoGetMessages_Output) UnmarshalCBORAt(data []byte, pos int) (int, error) {
	s.extraCBOR = nil
...
// ConvoGetMessages_Output_Messages is a union type.
...
func (u ConvoGetMessages_Output_Messages) MarshalJSON() ([]byte, error) {
	return u.AppendJSON(make([]byte, 0, 256))
}

func (u ConvoGetMessages_Output_Messages) AppendJSON(buf []byte) ([]byte, error) {
	...
	return nil, fmt.Errorf("cannot marshal empty union ConvoGetMessages_Output_Messages")
}

func (u *ConvoGetMessages_Output_Messages) UnmarshalJSON(data []byte) error {
	_, err := u.UnmarshalJSONAt(data, 0)
	return err
}

func (u *ConvoGetMessages_Output_Messages) UnmarshalJSONAt(data []byte, pos int) (int, error) {
...
func (u ConvoGetMessages_Output_Messages) MarshalCBOR() ([]byte, error) {
	return u.AppendCBOR(make([]byte, 0, 256))
}

func (u ConvoGetMessages_Output_Messages) AppendCBOR(buf []byte) ([]byte, error) {
	if u.ConvoDefs_MessageView.HasVal() {
		v := *u.ConvoDefs_MessageView.Val()
		v.LexiconTypeID = "chat.bsky.convo.defs#messageView"
...
func (u *ConvoGetMessages_Output_Messages) UnmarshalCBOR(data []byte) error {
	_, err := u.UnmarshalCBORAt(data, 0)
	return err
}

func (u *ConvoGetMessages_Output_Messages) UnmarshalCBORAt(data []byte, pos int) (int, error) {
...
func ConvoGetMessages(ctx context.Context, c *xrpc.Client, convoId string, cursor string, limit int64) (*ConvoGetMessages_Output, error) {
	...
	return &out, c.Query(ctx, "chat.bsky.convo.getMessages", params, &out)
}
