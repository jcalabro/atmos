...
// Precomputed JSON key tokens for ReportQueryReports_Output.
var (
	jsonKey_ReportQueryReports_Output_dollar_type = []byte("\"$type\":")
	jsonKey_ReportQueryReports_Output_cursor      = []byte("\"cursor\":")
	jsonKey_ReportQueryReports_Output_reports     = []byte("\"reports\":")
)

func (s *ReportQueryReports_Output) MarshalJSON() ([]byte, error) {
	return s.AppendJSON(make([]byte, 0, 256))
}

func (s *ReportQueryReports_Output) AppendJSON(buf []byte) ([]byte, error) {
	buf = append(buf, '{')
	first := true
	if s.LexiconTypeID != "" {
		if !first {
		...
		buf = append(buf, jsonKey_ReportQueryReports_Output_dollar_type...)
	...
	if s.Cursor.HasVal() {
		...
		buf = append(buf, jsonKey_ReportQueryReports_Output_cursor...)
	...
	buf = append(buf, jsonKey_ReportQueryReports_Output_reports...)
	buf = append(buf, '[')
...
func (s *ReportQueryReports_Output) UnmarshalJSON(data []byte) error {
	_, err := s.UnmarshalJSONAt(data, 0)
	return err
}

func (s *ReportQueryReports_Output) UnmarshalJSONAt(data []byte, pos int) (int, error) {
		...
		case "reports":
			if !cbor.IsJSONNull(data, pos) {
				...
				for {
					...
					var elem ReportDefs_ReportView
...
// Precomputed CBOR key tokens for ReportQueryReports_Output.
var (
	cborKey_ReportQueryReports_Output_dollar_type = cbor.AppendTextKey(nil, "$type")
	cborKey_ReportQueryReports_Output_cursor      = cbor.AppendTextKey(nil, "cursor")
	cborKey_ReportQueryReports_Output_reports     = cbor.AppendTextKey(nil, "reports")
)

func (s *ReportQueryReports_Output) MarshalCBOR() ([]byte, error) {
	return s.AppendCBOR(make([]byte, 0, 256))
}

func (s *ReportQueryReports_Output) AppendCBOR(buf []byte) ([]byte, error) {
	...
	if len(s.extra) > 0 {
		...
		if s.LexiconTypeID != "" {
			buf = append(buf, cborKey_ReportQueryReports_Output_dollar_type...)
			buf = cbor.AppendText(buf, s.LexiconTypeID)
		}
		ei, buf = appendCBORExtrasBefore(s.extra, ei, "cursor", buf)
		if s.Cursor.HasVal() {
			buf = append(buf, cborKey_ReportQueryReports_Output_cursor...)
			buf = cbor.AppendText(buf, s.Cursor.Val())
		}
		ei, buf = appendCBORExtrasBefore(s.extra, ei, "reports", buf)
		buf = append(buf, cborKey_ReportQueryReports_Output_reports...)
		buf = cbor.AppendArrayHeader(buf, uint64(len(s.Reports)))
...
func (s *ReportQueryReports_Output) UnmarshalCBOR(data []byte) error {
	_, err := s.UnmarshalCBORAt(data, 0)
	return err
}

func (s *ReportQueryReports_Output) UnmarshalCBORAt(data []byte, pos int) (int, error) {
	...
	for i := uint64(0); i < count; i++ {
		...
		case 7:
			if string(data[keyStart:keyEnd]) == "reports" {
				{
					...
					pos = newPos
					s.Reports = make([]ReportDefs_ReportView, arrLen)
					for idx := range arrLen {
						pos, err = s.Reports[idx].UnmarshalCBORAt(data, pos)
						if err != nil {
							return 0, err
						}
					}
...
type ReportQueryReports_Output struct {
	...
	Reports       []ReportDefs_ReportView `json:"reports"`

	// extra preserves unknown fields for same-format round-trips.
	extra []extraField
}

// ReportQueryReports calls the XRPC query "tools.ozone.report.queryReports".
//
...
func ReportQueryReports(ctx context.Context, c *xrpc.Client, assignedTo string, collections []string, cursor string, did string, isMuted bool, limit int64, queueId int64, reportTypes []string, reportedAfter string, reportedBefore string, sortDirection string, sortField string, status string, subject string, subjectType string) (*ReportQueryReports_Output, error) {
	...
	return &out, c.Query(ctx, "tools.ozone.report.queryReports", params, &out)
}
