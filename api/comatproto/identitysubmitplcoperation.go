...
// Precomputed JSON key tokens for IdentitySubmitPlcOperation_Input.
var (
	jsonKey_IdentitySubmitPlcOperation_Input_dollar_type = []byte("\"$type\":")
	jsonKey_IdentitySubmitPlcOperation_Input_operation   = []byte("\"operation\":")
)

func (s *IdentitySubmitPlcOperation_Input) MarshalJSON() ([]byte, error) {
	return s.AppendJSON(make([]byte, 0, 256))
}

func (s *IdentitySubmitPlcOperation_Input) AppendJSON(buf []byte) ([]byte, error) {
	buf = append(buf, '{')
	first := true
	if s.LexiconTypeID != "" {
		if !first {
		...
		buf = append(buf, jsonKey_IdentitySubmitPlcOperation_Input_dollar_type...)
...
func (s *IdentitySubmitPlcOperation_Input) UnmarshalJSON(data []byte) error {
...
// Precomputed CBOR key tokens for IdentitySubmitPlcOperation_Input.
var (
	cborKey_IdentitySubmitPlcOperation_Input_dollar_type = cbor.AppendTextKey(nil, "$type")
	cborKey_IdentitySubmitPlcOperation_Input_operation   = cbor.AppendTextKey(nil, "operation")
)

func (s *IdentitySubmitPlcOperation_Input) MarshalCBOR() ([]byte, error) {
...
func (s *IdentitySubmitPlcOperation_Input) UnmarshalCBOR(data []byte) error {
...
func IdentitySubmitPlcOperation(ctx context.Context, c *xrpc.Client, input *IdentitySubmitPlcOperation_Input) error {
	return c.Procedure(ctx, "com.atproto.identity.submitPlcOperation", input, nil)
}
